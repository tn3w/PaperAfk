...
public class AfkService {
    ...
    public void setupAfkWorld() {
        ...
        if (worldFolder.exists()) {
            ...
            if (!deleteWorldFolder(worldFolder)) {
                plugin.getLogger().warning("Could not completely delete the AFK world folder. Some files may remain.");
            }
        }
    ...
    public void setPlayerAfk(Player player) {
        ...
        if (isPlayerAfk(player)) {
            if (previousLocations.containsKey(playerId)) {
                ...
                player.teleport(Bukkit.getWorlds().get(0).getSpawnLocation());
                plugin.getLogger().warning("Previous location for player " + player.getName() + " not found, teleporting to spawn.");
            }
    ...
    private Location getOrCreateAfkRoom(Player player) {
        ...
        Location centerLocation = roomLocation.clone().add(configManager.getRoomSize() / 2, 1, configManager.getRoomSize() / 2);
    ...
    /**
     * Performs cleanup operations before the plugin shuts down
     * This helps ensure proper state for the next server start
     */
    ...
    private void createAfkRoom(Location location, String playerName) {
            ...
            for (int dz = 0; dz < roomSize; dz++) {
                for (int dy = 0; dy < 5; dy++) {
                    ...
                    if (dy == 0) {
                        ...
                        boolean isCorner = (dx < 4 && dz < 4) || (dx < 4 && dz >= roomSize-4) || 
                                          (dx >= roomSize-4 && dz < 4) || (dx >= roomSize-4 && dz >= roomSize-4);
        ...
        org.bukkit.block.data.type.Lantern lanternData = (org.bukkit.block.data.type.Lantern) afkWorld.getBlockAt(centerX, y + 3, centerZ).getBlockData();
        ...
        org.bukkit.block.data.Directional couchData1 = (org.bukkit.block.data.Directional) afkWorld.getBlockAt(couchLoc1).getBlockData();
        ...
        org.bukkit.block.data.Directional couchData2 = (org.bukkit.block.data.Directional) afkWorld.getBlockAt(couchLoc2).getBlockData();
        ...
        org.bukkit.block.data.type.Lantern tableLanternData = (org.bukkit.block.data.type.Lantern) afkWorld.getBlockAt(nightstandLoc.clone().add(0, 1, 0)).getBlockData();
        ...
        org.bukkit.block.data.type.Lantern soulLanternData = (org.bukkit.block.data.type.Lantern) afkWorld.getBlockAt(deskLoc.clone().add(0, 1, 0)).getBlockData();
        ...
        org.bukkit.block.data.Directional chairData = (org.bukkit.block.data.Directional) afkWorld.getBlockAt(chairLoc).getBlockData();
        ...
        org.bukkit.block.Jukebox jukebox = (org.bukkit.block.Jukebox) afkWorld.getBlockAt(jukeboxLoc).getState();
