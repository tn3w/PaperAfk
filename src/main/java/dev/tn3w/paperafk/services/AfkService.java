...
public class AfkService {
    ...
    public void setupAfkWorld() {
        ...
        if (afkWorld != null) {
            ...
            for (Player player : afkWorld.getPlayers()) {
                ...
                player.sendMessage(
                        "§cThe AFK world is being reset. You have been teleported to spawn.");
            ...
            if (Bukkit.unloadWorld(afkWorld, false)) {
                ...
                if (deleteWorldFolder(worldFolder)) {
                    ...
                    plugin.getLogger()
                            .warning("Failed to delete AFK world folder. Some files may remain.");
                }
            } else {
                plugin.getLogger()
                        .warning("Failed to unload AFK world. Trying to continue anyway...");
            }
        }
                            ...
                            () -> {
                                for (int x = -10; x <= 10; x++) {
                                    for (int y = -5; y <= 5; y++) {
                                        for (int z = -10; z <= 10; z++) {
                                            Block block =
                                                    afkLocation.getBlock().getRelative(x, y, z);
    ...
    /**
     * Performs cleanup operations before the plugin shuts down This helps ensure proper state for
            ...
            if (playerAfkStatus.get(playerId)) {
                ...
                if (player != null && player.isOnline()) {
                    ...
                    if (previousLocations.containsKey(playerId)) {
                        ...
                        player.sendMessage(
                                "§cYou have been returned from AFK due to server shutdown.");
                    } else {
                        player.teleport(Bukkit.getWorlds().get(0).getSpawnLocation());
                    }
                }
    ...
    private void createAfkRoom(Location location, String playerName) {
            ...
            for (int dz = 0; dz < roomSize; dz++) {
                for (int dy = 0; dy < 5; dy++) {
                    ...
                    if (dy == 0) {
                        ...
                        boolean isEdge =
                                dx == 0 || dx == roomSize - 1 || dz == 0 || dz == roomSize - 1;
        ...
        afkWorld.getBlockAt(tableLoc.clone().add(0, 1, 0))
                .setType(Material.DARK_OAK_PRESSURE_PLATE);
